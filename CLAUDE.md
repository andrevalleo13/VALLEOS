...
- **Modelo de datos** (5 tablas): `workout_routines` (rutinas, una `active`) → `workout_days` (días con `muscle_groups[]`, `day_order` define el ciclo) → `workout_exercises` (plantilla: `target_sets`/`target_reps`/`muscle_group`, **`tracking_type`** `'strength'|'timed'|'bodyweight'` default `'strength'`, `target_duration_seconds` para ejercicios por tiempo). Lo registrado: `workout_sessions` (un entrenamiento, guarda `day_name` denormalizado) → `workout_sets` (peso×reps por serie o `duration_seconds` para ejercicios `timed`, `exercise_name`/`muscle_group` denormalizados para sobrevivir ediciones)
...
- **Interactivo desde la app** (no desde el chat): `LogSession.tsx` registra una sesión (elige día → prefilla ejercicios → captura series; el input cambia según `tracking_type`: `strength` → kg×reps, `timed` → campo de segundos, `bodyweight` → solo reps; ejercicios `timed` y `bw` muestran badge en el header; permite ejercicio libre con selector de tipo). `RoutineEditor.tsx` (modal `wide`) gestiona rutinas/días/ejercicios con copia local de trabajo + persistencia inmediata; cada ejercicio tiene selector de tipo (Fuerza/Tiempo/BW) — al elegir Tiempo el campo de reps pasa a ser segundos objetivo. `router.refresh()` al cerrar
...
- **Web Push en background (PWA cerrada)**: stack completo implementado.
  ...
  - `app/api/cron/notify/route.ts` — cron que busca `notifications` con `pushed=false` y las empuja vía `pushToAll`, luego marca `pushed=true`. Corre cada 5 min en Vercel Pro (en Hobby solo 1/día).
...
- **Shadow empuja notis**: tool `crear_notificacion` → inserta en la tabla. El cron de notify la empuja al dispositivo en ≤5 min incluso con PWA cerrada.
...
- **Brief**: command center del día — prioridades (check/agregar/borrar), intención editable, toggle de hábitos, brief de Shadow, agenda del calendario (fetch client-side /api/calendar), finanzas del mes. Sección **"Atender hoy"** (radar): grid de tarjetas-señal que responde "¿qué tengo que atender?" — pagos ≤3 días, exámenes próximos con estado de estudio (urgent/study-now/soon), hitos de metas vencidos/próximos, follow-ups de Flouvia, gym "hoy toca". Estado vacío: "día despejado". Tarjetas extra: **Tiempo · hoy** (minutos registrados con `fmtHours`) y **Leyendo** (libro en progreso con barra de páginas). Helper: `lib/brief/today.ts` — `buildBriefRadar(supabase, today)` hace un `Promise.all` de ~12 queries y devuelve `{items: RadarItem[], tiempoHoy, libro}`, reusando `buildUpcomingPayments`, `studyState`, `milestoneState`, `daysUntil`
