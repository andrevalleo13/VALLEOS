...
export function Analysis({ initial, generatedAt }: { initial: string | null; generatedAt: string | null }) {
  ...
  async function run() {
    setLoading(true);
    try {
      const res = await fetch("/api/shadow/flouvia", { method: "POST" });
    ...
    <div className="card fin-analysis-card mb-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <button className="btn btn-ghost btn-sm" onClick={run} disabled={loading}>
          ...
          {loading ? "Analizando…" : text ? "Actualizar" : "Analizar Flouvia"}
        </button>
      </div>
      {text ? (
        ...
        <p style={{ color: "var(--mute)", fontSize: 13 }}>
          Pídele a Shadow que analice Flouvia: pipeline, MRR, oportunidades de cowork y los próximos movimientos.
        </p>
      )}
    </div>
  );
